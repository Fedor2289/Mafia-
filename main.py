...
class UserMiddleware(BaseMiddleware):
    async def __call__(self, handler: Callable, event: TelegramObject, data: dict) -> Any:
        ...
        if tg_user and session:
            data["user"] = await get_user(
                session, tg_user.id, tg_user.username, tg_user.first_name or "User"
            )
        return await handler(event, data)
    ...
    async def __call__(self, handler: Callable, event: TelegramObject, data: dict) -> Any:
        try:
            ...
            if isinstance(event, Message):
                ...
                try:
                    await event.answer("⚠️ Ошибка. Администраторы уведомлены.")
                except Exception:
                    pass
            elif isinstance(event, CallbackQuery):
                ctx = f"Callback | data='{event.data}'"
                try:
                    await event.answer("⚠️ Ошибка.", show_alert=False)
                except Exception:
                    pass
...
async def main() -> None:
    ...
    # ── ВАЖНО: сбрасываем вебхук если он был установлен ──────────────────────
    # Без этого polling не получает апдейты если раньше стоял вебхук
