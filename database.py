...
class User(Base):
    ...
    group_ai: Mapped[bool] = mapped_column(Boolean, default=True)   # отвечать ли в группах
...
async def get_user(session: AsyncSession, telegram_id: int, username: str | None, first_name: str) -> User:
    ...
    if u is None:
        ...
        changed = False
        if u.username != username: u.username = username; changed = True
        if u.first_name != first_name: u.first_name = first_name; changed = True
        if changed:
            await session.commit()
    return u
